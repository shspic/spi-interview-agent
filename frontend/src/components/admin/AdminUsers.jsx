  ...
  return (
    <div className="admin-panel">
      ...
      {action.user && <div className="confirmation-panel" role="dialog" aria-modal="true"><div className="confirmation-card">
        {error && <p className="inline-error" role="alert">{error}</p>}
        {action.type === "detail" && <><h3>用户详情</h3>{actionLoading ? <p>正在读取...</p> : userDetail && <><dl className="account-detail-grid"><div><dt>用户名</dt><dd>{userDetail.username}</dd></div><div><dt>状态</dt><dd>{userDetail.is_active ? "正常" : "停用"}</dd></div><div><dt>角色</dt><dd>{userDetail.is_admin ? "管理员" : "普通用户"}</dd></div><div><dt>文件 / 会话</dt><dd>{userDetail.file_count} / {userDetail.interview_session_count}</dd></div></dl><div className="usage-cell detail-usage">{(userDetail.today_usage || []).map((item) => <span key={item.usage_type}>{item.display_name} {item.used}/{item.limit}</span>)}</div></>}<div className="dialog-actions"><button type="button" className="secondary-button" onClick={closeAction}>关闭</button></div></>}
